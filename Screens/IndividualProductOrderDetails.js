...
const IndividualProductOrderDetails = ({ route, navigation }) => {
  ...
  const navigateToTrackOrder = () => {
    // Check if shipment_id exists in orderData
    const shipmentId = orderData?.data?.shipment_id || orderData?.data?.tracking_id || orderData?.data?.id;
    ...
    if (shipmentId) {
      ...
      Alert.alert(
        ...
        'Shipment tracking ID is not available yet. Please check back later.',
        [{ text: 'OK' }]
      );
    }
  };
  ...
  // Determine order status text and color
    ...
    if (delivery_status < 3) {
      Alert.alert(
        ...
        [
          ...
          { text: 'Track Now', onPress: navigateToTrackOrder },
        ],
      );
      return;
    }
