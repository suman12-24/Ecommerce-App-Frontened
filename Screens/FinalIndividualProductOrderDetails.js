...
import { StyleSheet, Text, View, Image, TouchableOpacity, ScrollView, StatusBar, ActivityIndicator, Alert, Animated, Dimensions, BackHandler, } from 'react-native';
...
const IndividualProductOrderDetails = ({ route, navigation }) => {
    ...
    const fetchOrderDetails = async () => {
        try {
            setLoading(true);
            const response = await axiosInstance.get(`/Suhani-Electronics-Backend/f_order_details.php?o_id=${orderId}&u_id=${userId}`);
    ...
    const handleCancelOrder = async () => {
        try {
            setLoading(true);
            const response = await axiosInstance.put('/Suhani-Electronics-Backend/f_order_cancel.php', {
                o_id: orderId
            });
            if (response.data && response.data.success) {
                ...
                Alert.alert(
                    'Error',
                    response?.data?.message || 'Failed to cancel order. Please try again later.',
                    [{ text: 'OK' }]
                );
            }
        }
    ...
    const getProgressSteps = () => {
        ...
        const steps = [
            { label: "Ordered", completed: true, icon: "shopping-cart" },
            { label: "Shipped", completed: deliveryStatus >= 1, icon: "local-shipping" },
            { label: "Out for Delivery", completed: deliveryStatus >= 2, active: deliveryStatus === 2, icon: "local-shipping" },
            { label: "Delivered", completed: deliveryStatus >= 3, active: deliveryStatus === 3, icon: "done-all" }
        ];
        return steps;
    };
    ...
    const getOrderStatusInfo = () => {
        const { order_status, delivery_status, order_cancel, return_refund } = orderData.data;

        if (order_cancel === 1) return { text: "Cancelled", color: "#FF0000" }; // Red
        if (return_refund === 1) return { text: "Returned/Refunded", color: "#FFA500" }; // Orange
        if (delivery_status === 3) return { text: "Delivered", color: "#008000" }; // Green
        if (delivery_status === 2) return { text: "Out for Delivery", color: "#FFD700" }; // Yellow/Gold
    ...
    const handleStatusAction = () => {
        ...
        if (order_cancel === 1) {
            Alert.alert("Order Cancelled", "This order has been cancelled. Contact customer support for more information.");
            return;
        }

        if (return_refund === 1) {
            Alert.alert("Order Returned", "This order has been returned. Refund is being processed.");
            return;
        }
        ...
        <>
            ...
            <Animated.View
                style={[
                    ...
                    { opacity: headerOpacity, transform: [{ translateY: headerTranslate }], backgroundColor: COLORS.primaryDark }]}
            >
            ...
            >
                        ...
                        <View style={styles.orderIdContainer}>
                            ...
                            <View style={styles.orderIdRow}>
                                ...
                                <TouchableOpacity onPress={handleCopyOrderId}>
                                    <MaterialIcons name="content-copy" size={18} color="rgba(255,255,255,0.8)" />
                                </TouchableOpacity>
                            </View>
                        </View>
                        <View style={styles.orderDateContainer}>
                            <Text style={styles.orderDateLabel}>{t('orderDate')}</Text>
                            <Text style={styles.orderDate}>{formatDate(orderData.data.order_date)}</Text>
                        </View>
                    </LinearGradient>
                </Animated.View>
                ...
                >
                    <View style={styles.statusOverviewContainer}>
                        ...
                        <TouchableOpacity
                            style={[styles.statusActionButton, { backgroundColor: statusInfo.color }]}
                            onPress={handleStatusAction}
                        >
                            <Text style={styles.statusActionText}>
                                {statusInfo.text === "Delivered" ? `${t('leaveReview')}` : `${t('trackOrder')}`}
                            </Text>
                            <MaterialIcons
                                name={statusInfo.text === "Delivered" ? "rate-review" : "location-searching"}
                                size={16}
                                color="#fff"
                            />
                        </TouchableOpacity>
                                    ...
                                    style={[
                                        styles.stepIndicator,
                                        {
                                            ...
                                            backgroundColor: step.completed
                                                ? (step.isCancel ? COLORS.error : COLORS.primary)
                                                : COLORS.background,
                            ...
                            {progressSteps.map((step, index) => (
                                <View
                                    ...
                                    style={[
                                        ...
                                        {
                                            left: `${(index / (progressSteps.length - 1)) * 100}%`,
                                            marginLeft: index === 0 ? 0 : index === progressSteps.length - 1 ? -80 : -40
                                        }
                                    ]}
                                >
                                    <Text style={[
                                        styles.stepLabel,
                                        {
                                            color: step.completed
                                                ? (step.isCancel ? COLORS.error : COLORS.text)
                ...
                >
                    ...
                    {orderData.order_products.map((product, index) => (
                        <View key={index} style={[styles.productContainer, index > 0 && styles.productDivider]}>
                            <View style={styles.productImageContainer}>
                                <Image
                                    ...
                                    resizeMode="contain"
                                />
                                <View style={[styles.quantityBadge, { backgroundColor: '#bfbfbf' }]}>
                                    <Text style={styles.productQuantity}>Qty: {product.quantity}</Text>
                                </View>
                            </View>
                            <View style={styles.productDetails}>
                                <Text style={styles.productName} numberOfLines={2}>{product.p_name}</Text>
                                <View style={styles.priceContainer}>
                                    <Text style={[styles.productPrice, { color: COLORS.primary }]}>
                                        {formatCurrency(product.selling_price)}
                                    </Text>
                                    <Text style={styles.regularPrice}>{formatCurrency(product.regular_price)}</Text>
                                </View>
                                ...
                                {parseFloat(product.regular_price) > parseFloat(product.selling_price) && (
                                    <View style={styles.savingsBadge}>
                                        <Text style={styles.savingsText}>
                                            {t('save')} {formatCurrency(parseFloat(product.regular_price) - parseFloat(product.selling_price))}
                                        </Text>
                                    </View>
                                )}
                    ...
                    <View style={styles.shippingContainer}>
                        <View style={styles.iconTextRow}>
                            ...
                            <View style={styles.shipping}>
                                <Text style={styles.shippingTitle}>{orderData.user_address.name}</Text>
                        ...
                        <View style={styles.deliveryEstimateContainer}>
                            ...
                            <View>
                                <Text style={styles.deliveryEstimateLabel}>{t('estimateDelivery')}</Text>
                    ...
                    <View style={styles.paymentContainer}>
                        <View style={styles.paymentMethodContainer}>
                            <View style={styles.paymentIconContainer}>
                                ...
                                >
                                    <AntDesign name="creditcard" size={20} color={COLORS.primary} />
                                </LinearGradient>
                            </View>
                            <View>
                                <Text style={styles.paymentMethodLabel}>{t('paymentMethod')}</Text>
                                <Text style={styles.paymentMethod}>{orderData.data.payment_mode}</Text>
                            </View>
                        </View>
                        ...
                        <View style={styles.paymentBreakdown}>
                            <View style={styles.paymentRow}>
                                ...
                                <Text style={styles.paymentValue}>{formatCurrency(orderData.data.sub_total)}</Text>
                            </View>
                            <View style={styles.paymentRow}>
                                ...
                                <View style={styles.paymentRow}>
                                    <View style={styles.discountLabelContainer}>
                                        ...
                                        <View style={styles.couponTag}>
                                            <Text style={styles.couponCode}>{orderData.data.discount_coupon_code}</Text>
                                        </View>
                                    </View>
                        ...
                        <View style={styles.paymentStatusContainer}>
                            <View style={[styles.paymentStatusBadge, { backgroundColor: COLORS.success + '20' }]}>
                                ...
                                <Text style={[styles.paymentStatusText, { color: COLORS.success }]}>{t('paymentSucess')}</Text>
                            </View>
                            <TouchableOpacity style={styles.invoiceButton}>
                                ...
                                >
                                    ...
                                    <Text style={{ padding: 8, fontSize: 14, fontWeight: '500', color: '#fff' }}>{t('viewInvoice')}</Text>
                                </LinearGradient>
                            </TouchableOpacity>
                        </View>
                    </View>
                ...
                {orderData.data.delivery_status < 2 && orderData.data.order_cancel == 0 &&
                    <Animated.View
                        style={[
                            styles.card,
                            {
                        ...
                        <View style={styles.cancelContainer}>
                            <View style={styles.iconTextRow}>
                                ...
                                <View style={styles.cancelInfo}>
                                    <Text style={styles.cancelTitle}>{t('cancelYourOrder')}</Text>
                                    <Text style={styles.cancelDetails}>{t('orderCancelActionUndone')}</Text>
                                    <Text style={styles.cancelDetails}>{t('processRefund')}</Text>
                                </View>
                            </View>
