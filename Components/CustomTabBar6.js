...
const AnimatedStar = ({ startPosition, delay, duration }) => {
    ...
    useEffect(() => {
        const randomX = Math.random() * 60 - 30; // Random value between -30 and 30
...
const CustomTabBar = ({ state, descriptors, navigation }) => {
    ...
    const handleTabLayout = (e, index, routeName) => {
        ...
        // Store cart button position for star animation
        if (routeName === 'Cart') {
            setCartButtonPosition({ x, y });
        }
    };
    ...
    const renderStars = () => {
        ...
        return Array.from({ length: 8 }).map((_, index) => (
            <AnimatedStar
                key={index}
                startPosition={cartButtonPosition}
                delay={index * 50}
                duration={800}
            />
        ));
            ...
            >
                ...
                {state.routes.map((route, index) => {
                    ...
                    const onPress = () => {
                        ...
                        } else if (['Home', 'Cart', 'Category', 'Account', 'Wishlist'].includes(route.name)) {
                            navigation.reset({
                                index: 0,
                        ...
                        >
                            <Animated.View
                                style={[
                                    ...
                                    route.name === 'Cart' && cartIconAnimatedStyle
                                ]}
                            >
                                <Ionicons
